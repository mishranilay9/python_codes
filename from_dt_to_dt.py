...
def add_md5(source_df):
    row_rdd=source_df.rdd.map(func)
...
def perform_delta_identification(current_df, previous_hash_df, primary_key):
    ...
    insert_records = previous_hash_df.alias('target').join(current_df.alias('source'),
                                                       [previous_hash_df[v] == current_df[v]
                                                        for v in primary_key_list], how="right_outer") \
    .filter(previous_hash_df['hash_id'].isNull()) \
    .select([F.col('source.' + xx) for xx in current_df.columns])\
    ...
    update_records = previous_hash_df.alias('target').join(current_df.alias('source'),
                                                       [current_df[v] == previous_hash_df[v]
                                                        for v in primary_key_list], how="inner") \
    .filter(previous_hash_df['hash_id'] != current_df['hash_id'])
    new_recs=update_records.select([F.col('source.' + xx) for xx in current_df.columns]) \
        .withColumn("from_dt", F.lit(current_dt)).withColumn("to_dt", F.lit(long_gt)).withColumn("active_flag",F.lit("A")).drop('hash_id')

    old_recs = update_records.select([F.col('target.' + xx) for xx in previous_hash_df.columns]) \
        .withColumn("to_dt", F.lit(current_dt)).withColumn("active_flag",F.lit("NA")).drop('hash_id')


    delete_records = previous_hash_df.alias('target').join(current_df.alias('source'),
                                                       [previous_hash_df[v] == current_df[v]
                                                        for v in primary_key_list], how="left_outer"). \
    filter(current_df['hash_id'].isNull()).select([F.col('target.' + xx) for xx in previous_hash_df.columns]) \
        .withColumn("to_dt",F.lit(current_dt)).withColumn("active_flag",F.lit("NA")).drop('hash_id')
...
def main():
    ...
    Publish_hashed_df_final = Publish_hashed_df.alias('target').join(publish_data1.alias('source'),
                                                           [Publish_hashed_df[v] == publish_data1[v]
                                                            for v in primary_key_list], how="inner")\
        .select([F.col('target.' + xx) for xx in Publish_hashed_df.columns]+[F.col('source.from_dt')])
